...
function decideTransferEligibility({ callerText = "", isFriendsFamily = false } = {}) {
  ...
  const explicitTry = /(can|could|would|will) you (?:please )?(?:try|get|reach|call) (?:tommy|dad|him)|please (?:try|get|reach|call) (?:tommy|dad|him)|try to get (?:tommy|dad|him)|put me (?:straight )?through|can i speak (?:to|with) tommy|need to speak (?:to|with) tommy (?:now|urgently|immediately)/i.test(text);
