function hardenQuoteBasePrompt(prompt) {
  ...
  const newVariation = '- EXISTING LCM CUSTOMER ASKING FOR EXTRA WORK OR A VARIATION: This branch OVERRIDES the standard residential quote flow. Treat it as a short quote/variation request, not a normal schedule enquiry. If needed, use lookup_job_schedule only to identify the existing job by address or job number. Collect ONLY the extra work requested, rough size if not already stated, finish if relevant, and timing preference. NEVER ask what is currently there, what needs removal, access, steepness, pumps, or unusual site conditions unless the caller independently raises that subject. Once those short variation details are known, summarise and move to callback confirmation.';
